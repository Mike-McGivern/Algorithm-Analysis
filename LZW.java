 ...
 * and flush in case we are on odd half. */
        ...
        outComp.flush ();
	System.out.println("Output size is " + outnum);
        return inctr;
    }
            ...
            try {
                BufferedInputStream bis
                 = new BufferedInputStream
                     (new FileInputStream
                         (new File (argv [argc])));
FileOutputStream s= new FileOutputStream(new File("Test"));
              CodeOutputPacker baos = new CodeOutputPacker (s);

                nComp = LZW.Compress (bis, baos);
FileInputStream s2 = new FileInputStream(new File("Test"));
                nExp = LZW.Expand (new CodeInputUnpacker
                           (s2),
                   System.out);
