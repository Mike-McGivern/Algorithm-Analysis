...
* default 12 bits. */ 
   ...
   { 
	this.bytes = new byte [nbytes]; this.width = N; 
   } 
...
* default 12 bits. */ 
   ...
   { 
	this.stream = stream; this.width = N; 
   } 
...
* @exception IOException possible only if based on a stream. */ 
   ...
   { 
	for (int ii = 0; ii < codes.length; ii++) putN (codes [ii]); 
   } 
...
* @return array sized for actual data contained. */ 
   ...
   private int masks[] = { 0x0000, 0x0001, 0x0003, 0x0007, 0x000F, 0x001F, 0x003F, 0x007F, 0x00FF, 0x01FF, 0x03FF, 0x07FF, 0x0FFF, 0x1FFF, 0x3FFF, 0x7FFF, 0xFFFF }; 
...
/* first or later code in a cycle */ 
	...
	if (acc_bits == 0) accum = code; 
	else accum = (accum << width) | code; 
